...
- If a session is active: resume it — read the task, call `TaskList` to see plan steps, check blockers
...
- Plan items are tracked as native Claude Code Tasks. Use `TaskCreate` for new steps, `TaskUpdate` to mark progress. For quick single-step tasks, skip `TaskCreate` — Tasks are for multi-step work decomposition only.
- `tasks-snapshot.md` is auto-generated by the cost-tracker hook — do not edit it.

## Session Lifecycle

After completing a task: transition to `idle`, do NOT close the session.
- Serialize and delete all native Tasks before invoking session-mgr for idle transition
...
- Run session: `/claude-code-hermit:hermit-session` — generic session workflow
- Start session: `/claude-code-hermit:hermit-session-start`
- Close session: `/claude-code-hermit:hermit-session-close`
- Session status: `/claude-code-hermit:hermit-state` — compact summary (auto-triggers on "status", "progress")
- Session brief: `/claude-code-hermit:hermit-brief` — executive summary (auto-triggers on "brief", "what happened")
- Monitor: `/claude-code-hermit:hermit-monitor` — session-aware monitoring loop
...
- Plan tracking lives in native Tasks, not SHELL.md — no plan table to manage
