// Read-only summary generator — reads state files, writes state-summary.md.
// No mutations to source files. Zero npm dependencies, Node stdlib only.
// Usage: node generate-summary.js <state-dir-path>
...
if (!stateDir) {
  console.error('Usage: node generate-summary.js <state-dir-path>');
  process.exit(1);
}
...
try {
  ...
  if (outMtime > Math.max(...sourceMtimes)) {
    process.exit(0);
  }
} catch {
...
try {
  ...
  if (skipFirst(content) === skipFirst(existing)) {
    process.exit(0);
  }
} catch {}
