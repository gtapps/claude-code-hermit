...
- If version is present but less than `1.0.17`: ask "Core v1.0.17+ required (found vX.Y.Z). Run `/claude-code-hermit:hermit-evolve` now?" using AskUserQuestion with options "Yes — upgrade now" / "Skip — proceed anyway". If yes: invoke `/claude-code-hermit:hermit-evolve`, then continue. If skip: proceed with a warning.
- If version is `1.0.17` or later: check that `.claude-code-hermit/state/runtime.json` exists. If missing, warn: "runtime.json not found — state may be incomplete. Re-run `/claude-code-hermit:hatch` or `/claude-code-hermit:hermit-evolve` to repair." Allow proceeding. Otherwise proceed.
