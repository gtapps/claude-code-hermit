...
2. Check if the operator provided an explicit intent:
   ...
   - `--idle`, "mission complete", or "idle" → **Idle Transition**
...
4. Confirm with the operator, showing the default: "Mission complete. [Idle transition / Full shutdown]?" In unattended mode (no operator response within context), use the default.

---

## Idle Transition (mission complete, session stays open)

Use this when the mission is done but the session should remain open for the next mission. Heartbeat, monitors, and channels continue running.

1. Use the `session-mgr` agent to finalize `.claude/.claude-code-hermit/sessions/ACTIVE.md`
2. Ensure all progress is recorded with step-level status (`done`, `blocked`, `planned`)
3. Document any blockers with enough context for the next mission to understand them
...
7. Use `session-mgr` to perform the **idle transition** (see "On Mission Complete" in session-mgr):
   - Archive the mission report as S-NNN-REPORT.md
   - Reset mission-specific sections in ACTIVE.md (Mission, Steps, Progress Log, Blockers, Discoveries, Changed)
   - Preserve session-scoped sections (Monitoring, Cost, Session Summary)
   - Set Status to `idle`, increment Missions Completed
   - Append one-line summary to Session Summary
8. Report to the operator: "Mission archived as S-NNN. Session idle — send a new mission via channel or type it here."
...
8. Archive the session via `session-mgr` (full close — replace ACTIVE.md with fresh template)
...
- [ ] Tags from ACTIVE.md are copied to the session report's Tags field
- [ ] Mission status is accurate (`completed` | `partial` | `blocked`)
...
**Idle transition:** skip the Next Start Point check — the session stays open and Session Summary captures the mission outcome.
