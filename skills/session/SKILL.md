...
6. Use `session-mgr` to perform an **idle transition** (archive report, reset task-scoped sections, set status to `idle`). session-mgr handles updating both SHELL.md (cosmetic) and `state/runtime.json` (lifecycle truth). Pass the task table in the prompt for the archived report.
