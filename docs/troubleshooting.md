...
- Check `state/runtime.json` (session state) — if stuck on `in_progress`, routines are queued to `state/routine-queue.json` (not skipped) and fire when session returns to idle. Inspect with: `cat .claude-code-hermit/state/runtime.json | jq .session_state`
