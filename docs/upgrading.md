...
## Version History (v0.3.2 – v0.3.10)

These versions added features automatically — no manual config migration required. Run `/claude-code-hermit:hermit-evolve` after updating to refresh templates and pick up any new config defaults.

| Version     | Key changes                                                                                                                                                                                                                                                 |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **v0.3.2**  | Session state moved from `.status` → `state/runtime.json`. SHELL.md `Status:` is now cosmetic — never read it programmatically. `tasks-snapshot.md` auto-generated by cost-tracker.                                                                         |
| **v0.3.4**  | `state/.heartbeat` activity marker (heartbeat-touch.js, PostToolUse). `state/channel-activity.json` added. `.lifecycle.lock` for always-on deployments.                                                                                                     |
| **v0.3.5**  | Contract test suite (`tests/run-contracts.py`, 20+ tests). `smoke-test` and `test-run` skills. `sessions/` and `proposals/` indexed for Cortex.                                                                                                             |
| **v0.3.6**  | `state/session-diff.json` for uncommitted file tracking. Cost log moved to `state/cost-log.jsonl`. Budget enforcement tightened.                                                                                                                            |
| **v0.3.7**  | Hook reorganization: 5 Stop hooks → 4 phases (PreToolUse/PostToolUse/SessionStart/Stop). Added `enforce-deny-patterns.js`, `channel-hook.js`, `heartbeat-touch.js`, `contract-test-runner.js`, `validate-config.js`. Added `routine-queue-flush.js` (Stop). |
| **v0.3.9**  | Hermit Cortex: `obsidian-setup`, `cortex-refresh`, `cortex-sync` skills. `state/cortex-manifest.json` for artifact indexing. `validate-frontmatter.js` test. `weekly-review` skill.                                                                         |
| **v0.3.10** | `cron-test-corpus.json` shared fixture for schedule-matching tests. Routine queue flush promotion to all profiles. Minor stability fixes.                                                                                                                   |

See [CHANGELOG.md](../CHANGELOG.md) for the full details of each release.
