...
| `git_tracking` | string | `"private"` | Git tracking mode: `"personal"` tracks hermit state in git (sessions, proposals, config, obsidian — recommended for solo projects); `"private"` gitignores it (recommended for teams). Determines which `.gitignore` template hatch stamps. Run `/hermit-migrate` before committing in personal mode to catch unprotected credentials. |
