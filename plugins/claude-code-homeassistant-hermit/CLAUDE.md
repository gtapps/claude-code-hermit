...
Native approval is the execution checkpoint for existing guarded actions. Preserve previews and validation; do not add a second conversational yes/no step. A denied request must not be retried through another tool or route.

The split between the two mechanisms is deliberate. Static `permissions.ask` rules cover the structural write verbs, and they match the documented `bin/ha-agent-lab` invocation only: a rule binds a literal spelling, so the `bun <root>/src/cli.ts` development form is not rule-covered. `cli-approval.ts` covers `call-service` and `restore-states` in both forms, because their sensitivity depends on the entity inside `--data`, which no glob can read. Reach for a rule when the command shape decides; reach for the hook when runtime data decides.
