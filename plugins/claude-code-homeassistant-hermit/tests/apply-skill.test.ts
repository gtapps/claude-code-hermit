...
test('HA apply instructions check policy and preview before the native approval invocation', () => {
  ...
  const approval = steps.findIndex(step => step.includes('Preview before native approval'));
  ...
  expect(steps[approval]!).toContain('Denial stops execution');
  expect(steps[approval]!).toContain('changed artifact or target requires a fresh preview and native approval');
});
