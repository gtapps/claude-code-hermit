...
- **`ha get-energy-prefs` / `set-energy-prefs <json>`** — read/replace the energy dashboard configuration (WS `energy/get_prefs`, `energy/save_prefs`), gated. Takes the full nested preferences object as JSON — the schema (energy sources, per-device consumption) is too complex for individual flags.
- **`ha reload-entry <entry_id>` / `disable-entry <entry_id> --disabled true|false`** — config-entry actions (REST reload, WS `config_entries/disable`), gated. `reload-entry` is this CLI's first REST-based gated write, extracted to a named handler with its own audit report to match the shape of every other gated write. Completes Phase 6 (config-entry helpers are deferred — see Boundary in the roadmap doc).
