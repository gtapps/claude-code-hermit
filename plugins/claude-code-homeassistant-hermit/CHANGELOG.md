...
- **`ha export-automation` / `ha export-script` CLI commands** — fetch a live HA automation or script config and write it to `.claude-code-hermit/raw/<domain>-<slug>.yaml`. Closes the missing read-from-HA direction; previously the toolchain was one-way (draft YAML → simulate → apply). The exported YAML is structurally equivalent to the live config but not byte-identical to a human-formatted file (`yaml.safe_dump` strips comments and authoring whitespace; key order is preserved). ID validation rejects empty and entity-id-shaped values; the filename component is slugified to guard against path separators, while the original id is still used verbatim for the HA REST call. Six new tests under `tests/test_cli_export.py` cover happy path, both domains, entity-id-shape rejection, 400-on-missing, filename slugification, and round-trip through `simulate.evaluate_yaml_policy`.
- **`compiled/acknowledged-violations.md` — operator-curated audit suppression** — new optional file at `.claude-code-hermit/compiled/acknowledged-violations.md` (template in `state-templates/compiled/`, `foundational`-tagged). `audits.py` reads it deterministically: each `automation_ids[]` entry is paired with a body bullet `` - `<id>`: refs=[<entity_or_service>, ...]; <rationale> ``. Suppression fires only when the automation's current sensitive refs are a **subset** of the declared refs — so future drift (an acknowledged automation gaining a NEW sensitive ref) re-surfaces as a normal finding. Suppressed entries move to `acknowledged[]` in the JSON sidecar with `source`, `quoted_line`, and the original reasons; stdout findings count uses post-filter `len(violations)`. Hatch does not seed the file — operators copy the template when they have something to acknowledge; the loader fails open on missing/malformed input. `knowledge-schema.md` and `CLAUDE.md` Memory Conventions document the format. **Does NOT bypass any runtime gate** — `ha-apply-change` and the MCP safety hook still enforce `ha_safety_mode` exactly as before.
- **Auto-memory cross-check in `ha-morning-brief` and `ha-integration-health`** — both skills now consult the operator's auto-memory `MEMORY.md` and suppress entity-level bullets for IDs listed under a `## Known Issues` / `## Known Broken` section. Filters every entity bullet, not just unavailable ones (covers error states like `sensor.pm25 == -1`). For integration-health the filter applies *before* per-domain ratio computation, so a known-broken vacuum doesn't push the `vacuum` domain over the 50% threshold. Suppression is summarised as `(N suppressed by auto memory)` at the bottom of the output; suppressed IDs are never listed by name. `CLAUDE-APPEND.md` Core Rules gains a one-liner telling operators to keep these IDs at the top level of MEMORY.md (topic files don't auto-load, so they can't be relied on for this filter).
