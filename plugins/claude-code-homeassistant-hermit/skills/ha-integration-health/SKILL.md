...
description: Detect dropped HA integrations by computing per-domain unavailable-entity ratios from the latest context snapshot. Flags domains where most entities are unavailable, suggesting the integration lost its connection. Runs daily as a routine via reflect --check-id ha-integration-health --check claude-code-homeassistant-hermit:ha-integration-health.
allowed-tools:
  - Bash
---
