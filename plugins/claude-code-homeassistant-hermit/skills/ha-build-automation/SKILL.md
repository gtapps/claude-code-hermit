...
3. **Provision missing helpers**:
   ...
   - Run `${CLAUDE_PLUGIN_ROOT}/bin/ha-agent-lab ha create-helper <type> '<json>'` and handle the result:
     - `"requires_confirm": true`, `ha_safety_mode` is `ask`; describe the helper, then invoke with `--confirm` for native permission. Do not ask for another chat confirmation.
