...
const [MICRO_PREFIX, PROPOSAL_PREFIX] = ['micro-proposal-pending:', 'proposal-pending:'];
const STRUCTURED_PREFIXES = [MICRO_PREFIX, PROPOSAL_PREFIX];
...
function apply(payloadJson: string): void {
  ...
  const modelFiring = validated.filter(f => !STRUCTURED_PREFIXES.some(p => f.key.startsWith(p)));
  ...
  const updated = {
    ...
    alerts,
    self_eval,
    last_clean_eval_at: result.lastCleanEvalAt,
  ...
  process.stdout.write(JSON.stringify({
    ...
    heartbeat_result: result.heartbeatResult,
  }) + '\n');
