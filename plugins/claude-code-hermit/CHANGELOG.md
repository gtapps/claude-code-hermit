...
- **hook contract tests on bun test** — `tests/run-hooks.sh` (896 lines of bash) replaced by `tests/hooks.contract.test.ts` + shared helpers; all 82 cases preserved, subprocess spawning kept only at the actual hook process boundary (stdin → exit code/stdout, fail-open), unit probes now in-process. ~3.5s vs minutes.
