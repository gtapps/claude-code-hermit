...
- **Suggestion cards: plain-language proposal approvals on channel-tagged turns** — `proposal-list` renders open proposals as "Suggestion #N … Reply YES/LATER/NO" cards on a channel-tagged turn instead of the PROP-NNN/tier/age table; `channel-responder` maps YES/LATER/NO (and a bare `#N`) to accept/defer/dismiss without the operator ever typing a PROP id; `proposal-act` confirms in the same plain voice ("Got it — starting on Suggestion #N"). `#N` is the proposal's existing integer, not a new counter. Terminal output (`/proposal-list`, direct `/proposal-act`) is unchanged.
### Changed
