...
- **Operator-configurable primary outbound channel (`channels.primary`, PROP-041).** Adds `scripts/resolve-outbound-channel.js`, a testable CLI that resolves the best eligible channel for proactive sends: checks `channels.primary` first (if set and reachable), then falls back to fixed priority `discord` → `telegram` → `imessage`. A channel is eligible if `enabled !== false`, `allowed_users` is not `[]`, and `dm_channel_id` is set. `weekly-review` now calls this resolver instead of inlining the priority list. `validate-config.js` special-cases `channels.primary` as an optional string that must reference an existing channel key. The Operator Notification protocol in `CLAUDE-APPEND.md` is updated to document the resolver and the revised eligibility rule (the prior "exactly one allowed user" requirement is replaced by the dm_channel_id-based unambiguity check, which is accurate for how proactive sends actually work).
