...
- **Binding `AskUserQuestion` guard on unattended sessions** — new `config.ask_gate` (default `true`) plus a `PreToolUse` hook (`scripts/ask-gate.ts`) deny `AskUserQuestion` on an `always_on` session with a reachable channel and redirect the model to the channel reply tool and a durable micro-proposal entry, so a spontaneous ask from a built-in flow, another plugin's skill, or the model itself can't silently stall an unwatched pane. The watchdog gains a companion check for prompts that can't be redirected this way (native permission dialogs) — a single deduped operator notification per stalled episode, never an auto-answer. See [Unattended Asks](docs/security.md#unattended-asks).
