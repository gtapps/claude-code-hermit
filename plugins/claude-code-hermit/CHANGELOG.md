...
- **procedure capture: reflect drafts a skill from recurring procedure, proposes it as an operator-gated Tier-3 capability PROP, and proposal-act installs it to `.claude/skills/` on accept** — when a multi-step procedure recurs in ≥2 sessions with no existing skill covering it, reflect writes a concise `procedure-brief` (type: procedure-brief) to `compiled/` as an audit record, routes a `category: capability, tags: [procedure-capture]` proposal through the standard triage→judge→proposal-create pipeline, and on accept invokes `/skill-creator` to author the final SKILL.md, then presents it for explicit operator confirmation before installation. Kill criteria: ≥8 candidates, triage-survival ≥25%, acceptance ≥30% (per candidate surfaced). Governed by the same metric-driven kill criteria as the brainstorm skills.
