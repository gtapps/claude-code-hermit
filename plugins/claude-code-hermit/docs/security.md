...
- **`permissions.allow: ["Artifact"]` is a sanctioned pre-approval of an outward-publishing tool for autonomous use.** Offered by `hatch`/`hermit-evolve` so unattended sessions don't stall on the first-publish permission ask (a headless "ask" is an effective deny). Assessment: on Pro/Max, artifacts are private to the publishing account with no share option, so a prompt-injected publish is persistence/noise, not exfiltration — the attacker cannot view the page, and `channel-send.ts` (above) remains the materially riskier egress. The real cost is losing the human checkpoint before irreversible external persistence: there is no tool-side delete, and published content may be cached even after removal from the gallery, so a mistaken publish (e.g. a render that embeds a secret that leaked into state) can't be fully recalled. Mitigation: every automatic-path publish is a deterministic render of enumerated state files (no free-form model-authored pages on the automatic path — see the brief-text caveat above); operators who decline the allow rule get the same outcome via setup-time banking (first publish happens inline, attended, at hatch/evolve time) instead of a standing grant.
