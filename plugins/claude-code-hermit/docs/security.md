...
| Advanced hardening (opt-in) | docker-compose.security.yml | Kernel-enforced (LAN block + DNS policy, read-only root, resource bounds, sysctls — opt-in via `/docker-security`; LAN containment is the load-bearing layer here) |
