...
- **Everything else (async approval + fail-loud, deterministic):** built-in flows, other installed plugins' skills, and spontaneous model-initiated questions are invisible to that static scan. A PreToolUse hook (`scripts/ask-gate.ts`, matcher `AskUserQuestion`) denies the call on a channel-primary session with a redirect reason that steers the model to the channel reply tool plus a durable `micro-proposals.json` entry — deny-steering behavior is probe-verified, the model does not retry the tool. Prompts that can't be redirected this way (native permission dialogs, harness-rendered prompts below the tool layer) are instead caught by the watchdog, which recognizes a stalled dialog on the captured pane and pushes one deduped operator notification per episode. Opt out of the gate with `ask_gate: false`; the watchdog check is unconditional on any `watchdog.enabled` hermit.
