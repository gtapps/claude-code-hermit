...
Yes. Docker is recommended for always-on mode but not required. Use `hermit-start`/`hermit-stop` for bare tmux. For interactive-only use, just run `/claude-code-hermit:steer` — no tmux or Docker needed.
