...
Monitor scheduling applies one global lateness limit to all scheduled routines. After downtime or a long busy turn, the latest missed occurrence runs only if it is within `routine_max_lateness_minutes` of the current minute. Exactly the limit remains eligible; older occurrences are consumed as `skipped-late` before prechecks or model wakes. Missed occurrences still collapse into one run per routine within the existing 24-hour search window. Pause/waiting skips retain precedence. This is a dispatch-time check, not a deadline on already dispatched work.
...
Native settings that config.json does not model go in the optional operator-owned `.claude-code-hermit/claude-settings.json`. Generated settings win; env merges by key with resolved Hermit values winning. Project-wide settings stay in `.claude/settings*.json`.
