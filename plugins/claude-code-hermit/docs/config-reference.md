...
| `quality_gate` | object | `{"enabled": true}` | Post-implementation quality review. When `enabled` is not explicitly `false`, `/proposal-act` runs `/simplify` on the implementation before resolve. Tune via `/hermit-settings quality-gate`. |
