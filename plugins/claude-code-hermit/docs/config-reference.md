...
| `language` | string | `null` | Preferred language for responses. Auto-detected from locale. Also applies to script-composed channel messages (budget, denial, mint, watchdog, status), not just model replies. Boot owns only the local `language` key in `.claude/settings.local.json`: it mirrors this value when set and removes that local key when unset. The `/config` Language picker writes user scope and survives restart. See the [access model](access-model.md#preferences). |
