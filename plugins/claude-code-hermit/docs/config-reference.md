...
| `passive_chats` | string array | _(absent, off)_ | Chats recorded for recall while waking only for mentions from allowed senders. Requires plugin `requireMention:false` and empty per-chat `allowFrom`. Telegram requires privacy mode disabled in BotFather; forum topics share the group id. Discord threads follow the listed parent; forum channels are unsupported. Denying Create Public/Private Threads is the zero-code alternative. A listed DM id is just blocked. Reply-to-bot without a mention and plugin `mentionPatterns` do not wake in a passive chat, and neither do the five control commands — `<@bot-id> /pause` binds, bare `/pause` is swallowed. Needs `bot_user_id` (nothing can match a mention without it) and `allowed_users` (absent, every member who mentions the bot wakes it); `hermit-doctor` warns for both. |
