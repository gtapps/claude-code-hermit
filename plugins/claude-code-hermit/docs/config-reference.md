...
DST transitions self-correct within 24h via the daily `heartbeat-restart` reload. **On the DST transition day itself, one CronCreate-mode fire may land at the wrong wall-clock hour** (monitor mode has its own, unrelated DST edges — see the routine-monitor design notes). If your `config.timezone` is null, schedules pass through unchanged (machine-local time used as-is).
