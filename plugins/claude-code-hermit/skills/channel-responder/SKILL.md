...
- **Thread request**: handle a direct request to open a thread, without `!fork` syntax or other work attached, in the resident alone, from any chat: it is not a task, so **Bind** does not apply. Check the current channel's loaded MCP tools, deriving the plugin and server segments from raw `source` as in §0: if a thread-creation tool is present, call it per its own schema, anchored on the inbound message where the schema accepts one, and reply with whatever the result names. Otherwise, on Discord, run `bun ${CLAUDE_PLUGIN_ROOT}/scripts/conversation.ts .claude-code-hermit chat-lookup --chat-id '<chat_id>'`. For `"thread":true` (types 10/11/12), reply “already a thread”; for DMs (1/3) or unsupported types, including forums, reply “can't open one from this channel type”. Report an `ERROR|` lookup plainly and stop. For type 0/5, run `bun ${CLAUDE_PLUGIN_ROOT}/scripts/conversation.ts .claude-code-hermit thread-create --chat-id '<chat_id>' --message-id '<message_id>' --name '<title>'` with a 1-100 character title, using the inbound `message_id` or the quoted message's id. On `OK|<id>`, reply with `https://discord.com/channels/<guild_id>/<id>`; on `ERROR|`, report the failure plainly with the likely cause (the bot lacks Create Public Threads), without retrying. If neither a plugin tool nor the Discord script applies, reply in one line that this channel has no thread creation, with no tool or script call. End after handling the request: never `spawn-session`, never `bind` or `update`, and no git or `permission_mode` precondition on any branch.
...
- **Question** ("why did you...", "what about...", "how does X work?")
  ...
  - Answer capability questions from the loaded channel tool list plus the script-backed commands this skill documents for the current platform. Distinguish “supported” from “the bot has permission here” (unknown until attempted) and “done” (only after a success result). If neither a plugin tool nor a script path supports the feature, say it is not supported on this channel rather than naming a platform.
