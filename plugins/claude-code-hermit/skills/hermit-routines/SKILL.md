...
```
/claude-code-hermit:hermit-routines load              diff-register enabled config.routines as CronCreates
/claude-code-hermit:hermit-routines load --reset      unconditional reset: tear down + recreate every enabled routine
...
1. Resolve the plugin root path: derive it from this skill's **Base directory**, which the harness injects into the invocation context as `<plugin_root>/skills/hermit-routines`. Strip the trailing `/skills/hermit-routines` to get `pluginRoot`. This works in both installed and `--plugin-dir` modes. (`$CLAUDE_PLUGIN_ROOT` is NOT a Bash env var at runtime — evaluating it in Bash always returns empty. The braced `${CLAUDE_PLUGIN_ROOT}` form is text-substituted in skill markdown only in installed mode. Neither is reliable here — always use the Base-directory derivation.) The resolved `pluginRoot` must be baked into each routine's prompt at registration — it is not available inside cron-delivered prompts.
   ...
   **Validate `pluginRoot` before proceeding.** If `pluginRoot` is empty or `<pluginRoot>/scripts/log-routine-event.sh` or `<pluginRoot>/scripts/routine-precheck.ts` does not exist (`test -f`), abort `load` immediately — do **not** run Step 3 or register/delete any CronCreate — and log one line: `Routine load aborted: plugin scripts not found at "<pluginRoot>". No routines registered or reset.` This is a whole-`load` abort, not the per-routine isolation of Step 4: a bad `pluginRoot` breaks every routine's baked paths, and aborting before Step 3 protects the prior session's CronCreates from being torn down and left unreplaced.
2. Read `config.routines`, filter `enabled: true`. If none, log "No enabled routines in config." and stop.
3. Run the diff planner: `bun <pluginRoot>/scripts/cron-registry.ts plan .claude-code-hermit <pluginRoot>` via Bash (append ` --force` when invoked as `load --reset` — see below). Parse its stdout lines: `DELETE:<id>`, `CREATE:<id>|<schedule>`, `WARN:<id>|<reason>`, `KEEP:<n>`, and an optional trailing `WAKESPREAD:<distinct>|<max>|<loneliest>` (the wake-clustering lint — see Step 6).
   - **If the first line is `SKIP|<reason>`** (the planner fell open on an error — e.g. an unreadable/corrupt `config.json`): abort `load` and log one line: `Routine load aborted: cron planner failed — <reason>. No routines registered.` Do **not** fall through to a no-op — a planner failure must surface, not silently leave routines unregistered. (Fix the underlying problem, then run `load --reset` to force a full sweep.)
   - **If there are no `DELETE:`/`CREATE:` lines** (only `KEEP:<n>`, optionally a trailing `WAKESPREAD:` line): registration is already current. Skip Steps 4-5 — log `Routines unchanged: <n> current, 0 registered.`; if a `WAKESPREAD:` line is present, also emit the wake-spread warning on its own line (same wording as Step 6). Then stop. No `CronList`, no `CronCreate`, no `CronDelete` this run.
   - **Otherwise**, if there are any `DELETE:` lines: call `CronList` once, then for each `DELETE:<id>` call `CronDelete` on the entry whose prompt contains `[hermit-routine:<id>]` (skip silently if `CronList` has no such entry — a delete for an id the planner tracked but CC already forgot is a no-op, not an error).
4. For each `CREATE:<id>|<schedule>` line, build the prompt string (see templates below) using `<schedule>` from that line directly as the `cron` value — it is already timezone-shifted; no per-routine `cron-tz-shift.ts` Bash call is needed here (the planner already ran it). Then call `CronCreate`:
   - `cron`: `<schedule>` (from the `CREATE:` line)
   - `recurring`: true
   - `durable`: false
   - `prompt`: the resolved prompt string

   **Per-routine error isolation:** if `CronCreate` throws for one routine (bad cron expression, hit the 50-task session limit, etc.), record the failure and continue with the next routine. Do not abort the loop — one bad config entry must not prevent unrelated routines from registering. Track which ids actually succeeded — Step 5 needs the exact list.
5. Commit the mirror: `bun <pluginRoot>/scripts/cron-registry.ts commit .claude-code-hermit <pluginRoot> "<comma-separated ids that succeeded in Step 4>"` via Bash (empty string if none succeeded). This stamps `registered_at` — resetting the expiry-age clock — only for ids actually `CronCreate`'d in Step 4, and refreshes the mirror's boot marker.
6. Log one line summarizing outcomes: `Routines registered: <N> ok, <M> failed, <K> kept[, <W> tz-warned]`, where `N`/`M` are counts of `CREATE:` lines that succeeded/failed as `CronCreate` calls and `K` is the planner's `KEEP:<n>` value. If `M > 0`, list each failed id with its error on its own line. If any `WARN:` lines were present, list each with its reason on its own line. If a `WAKESPREAD:<distinct>|<max>|<loneliest>` line was present, append one advisory line: `WARN: wake spread — <distinct> distinct 30-min wake windows (max <max>); consider clustering: <loneliest>`. This is a wake-count nudge (fewer distinct wake windows ≈ fewer cache-cold wakes) — advisory only; it never blocks registration.

**`load --reset`:** the unconditional escape hatch for suspected mirror/reality drift (e.g. a CronCreate deleted outside this skill). Step 3's planner call gets `--force` appended, which returns every enabled routine as `CREATE` with no `DELETE` lines — but that alone doesn't clear anything already live, so `--reset` inserts an unconditional sweep before Step 4: call `CronList`, then `CronDelete` every entry whose prompt contains `[hermit-routine:`. **Step 5's `commit` call also gets `--force` appended** (`... commit .claude-code-hermit <pluginRoot> "<csv>" --force`) so the commit-time replan is forced too — otherwise, when the boot id already matches (the mid-session drift-recovery case `--reset` exists for), the unforced commit replan re-derives every routine as `KEEP` and carries its pre-reset (stale) `registered_at` forward, leaving the age clocks wrong right after a reset that actually recreated every cron. With `--force` on both calls, Steps 4-6 proceed against the forced all-`CREATE` set and repopulate the mirror fresh, with `registered_at` stamped now for every recreated routine.
...
```
This mirrors the scheduled-reflect gate below: the precheck runs once in bash against a content hash of SHELL.md's `## Findings`/`## Blockers`, so a `reflect_after` fire with no new candidates since the last processed quick run never loads the reflect skill body. A manual `/claude-code-hermit:reflect --quick` (not chained through a routine) bypasses this append entirely and always loads the skill, as today — see reflect/SKILL.md's quick-mode section for how it gets a deterministic hash of its own via `--force`.

**Special case — the routine's `skill` is exactly `claude-code-hermit:reflect`** (the scheduled full-reflect routine; not `--quick`/`--scheduled-checks` variants): reflect's body should not load on days with nothing to reflect on, so gate it in the prompt. Replace the `invoke /<skill>` clause with:
```
...
- **7-day auto-expiry depends on `heartbeat-restart`.** CC's recurring-task auto-expiry is a hard 7-day-from-creation cliff — it resets only when a routine is actually re-`CronCreate`'d, not by `CronList`/activity. `cron-registry.ts`'s diff planner (Step 3) re-registers any routine whose age crosses a conservative threshold (well under 7 days) even when its config is unchanged, so daily re-arm keeps every routine's clock reset without a full unconditional re-create. The `heartbeat-restart` routine fires daily and re-invokes `/claude-code-hermit:hermit-routines load`, which is what actually crosses that threshold. **If you disable `heartbeat-restart`, routine CronCreates expire after 7 days** — re-enable it, or run `/claude-code-hermit:hermit-routines load --reset` weekly by hand.
- **`state/cron-registry.json` is a derived mirror, not a source of truth.** It exists only to avoid redundant `CronList`/`CronCreate`/`CronDelete` calls; `load --reset` (or a missing/corrupt mirror, or a `state/.boot-id` mismatch after a restart) always falls back to treating every enabled routine as needing registration. Never hand-edit it.
