...
3. Run the diff planner: `bun <pluginRoot>/scripts/cron-registry.ts plan .claude-code-hermit <pluginRoot>` via Bash (append ` --force` when invoked as `load --reset` — see below). Parse its stdout lines: `DELETE:<id>`, `CREATE:<id>|<schedule>`, `WARN:<id>|<reason>`, `KEEP:<n>`.
   ...
   - **If the only line is `KEEP:<n>`** (no `DELETE:`/`CREATE:` lines): registration is already current. Skip Steps 4-5 — log `Routines unchanged: <n> current, 0 registered.` and stop. No `CronList`, no `CronCreate`, no `CronDelete` this run.
