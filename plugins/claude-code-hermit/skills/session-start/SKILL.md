...
3c. **Conversation reconciliation.** Silently run `claude agents --json | bun ${CLAUDE_PLUGIN_ROOT}/scripts/conversation.ts .claude-code-hermit prune`, then read `bun ${CLAUDE_PLUGIN_ROOT}/scripts/conversation.ts .claude-code-hermit list`. Re-subscribe `/claude-code-hermit:watch session <session_name>` for each record still `running` or `idle`, without an operator announcement. Leave `parked` and `unknown` records for their next incoming message. An empty or unparsable agents response changes nothing; never treat it as evidence all helpers stopped.
