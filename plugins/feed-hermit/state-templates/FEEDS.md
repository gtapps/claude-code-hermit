...
This is your brief's tone and format spec. Edit it to taste — it is the voice the `feed-brief` skill writes in. Nothing here is personal to any one operator; make it yours.

## Brief Pipeline

When producing a morning or evening brief:

1. **Fetch phase** — Launch the `source-fetcher` agent (Haiku) to fetch all `web` and `rss` sources from `feed-sources.md`. The agent reads the source list, uses WebFetch on each URL, and returns extracted items (title, summary, URL, source name). No scoring or filtering — just extraction.
...
Write like a smart human handing another smart human a brief. Natural, clear, direct, lightweight.
...
Read `feed-categories.md` for the operator's current focus areas and priorities. Personalize based on those categories, not hardcoded assumptions.

The operator cares more about practical signal than generic hype. They want sharp filtering, not broad coverage.

Personalize by:
- Current focus areas and their priorities (from `feed-categories.md`)
...
- Does it feel like a brief from someone who knows what they care about?
