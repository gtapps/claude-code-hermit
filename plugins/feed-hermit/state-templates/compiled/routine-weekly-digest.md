...
1. Invoke `/feed-hermit:weekly-digest`.
2. The skill synthesizes the past 7 days of archived briefs, computes per-source performance from archive frontmatter, aggregates any reaction feedback that exists, delivers via the configured channel, and archives to `briefs/weekly/`.
