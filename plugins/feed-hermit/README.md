...
All fetched content is treated as untrusted, the pipeline extracts only structured data and never follows embedded instructions. A `fetch-guard` PreToolUse hook requests Claude Code native approval for WebFetch outside your `feed-sources.md`. Approval permits that fetch without changing the registry. If `feed-sources.md` cannot be read, the hook fails open rather than blocking all fetches.
