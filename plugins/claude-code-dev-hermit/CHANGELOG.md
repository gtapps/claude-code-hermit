...
- **`/dev-pr` Gate 2 renders a `### Gate 0 Override` audit section in the PR body when `state/last-test.json.bypass` is present.** When the bypass field is absent the Verification section renders as today (`Tests: **pass**`). When it is present, Gate 2 instead renders `Tests: **audited override** (reason: <bypass.reason>)` followed by a `### Gate 0 Override` block with `base_sha` and `summary` from the bypass field — so reviewers can see exactly what was knowingly bypassed. The `reason` value is emitted directly from the field, so any future addition to the `bypass.reason` enum surfaces in the PR body automatically with no SKILL.md change. This is the mechanical half of the Gate 0 override protocol. Refs #91.

### Documentation

- **Documented an audited override protocol for `/dev-pr` Gate 0 pre-existing base failures.** New `docs/GATE-0-OVERRIDE.md` covers when to use it, the ephemeral-bypass warning, the four-step `jq`-based protocol, and what not to do. This is a paper-trail improvement, not a logic fix — Gate 0 still measures absolute pass; the bypass makes the workaround deliberate and reviewer-visible instead of a silent file edit. `SKILL.md` Gate 0 step 3 and `state-templates/CLAUDE-APPEND.md` §Tests Before PR updated to point at the protocol; `docs/WORKFLOW.md` §Step 6 gains a cross-reference. Refs #91.

- **`docs/GATE-0-OVERRIDE.md`: protocol refinements.** Step 1 adds a `git checkout -t origin/<base>` fallback for fresh worktrees where the base isn't a local branch. New `## Nested-repo flow (`--cwd`)` subsection consolidates the nested-repo contract for the whole protocol: defines that bash blocks show the no-`--cwd` form, points at `skills/dev-pr/SKILL.md:27-33` for the full contract, and notes that `last-test.json` stays parent-scoped while `bypass.base_sha` (Step 3) must come from the child repo. `bypass.reason` is now documented as a closed enum. Drops a planning-leftover line from the "Next iteration" section. Refs #91.
