...
- **`docs/GATE-0-OVERRIDE.md`: protocol refinements.** Step 1 adds a `git checkout -t origin/<base>` fallback for fresh worktrees where the base isn't a local branch. Step 3 adds a nested-repo SHA note: the state-store path stays parent-scoped per `SKILL.md:29`, but `BASE_SHA` must come from the child repo via `git -C "<path>" rev-parse`. `bypass.reason` is now documented as a closed enum with a lockstep-update warning, and `skills/dev-pr/SKILL.md` Gate 2 gains a matching coupling note tying the hardcoded verification suffix to the enum value. Drops a planning-leftover line from the "Next iteration" section. Refs #91.
