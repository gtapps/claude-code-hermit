...
name: news-brief
description: >-
  Runs the full news briefing pipeline — fetches every configured source, scores
  and filters items by relevance, writes a human-voice brief per BRIEFING.md, delivers
  it to the operator's configured channel, and archives it with rich frontmatter.
  Invoke with /briefing-hermit:news-brief --morning|--evening|--slot <name> for
  scheduled or on-demand briefings. Reads sources.md, categories.md, and BRIEFING.md
  from the project root.
---

# News Brief

Run the full 7-phase briefing pipeline for this project. Config lives at the project root
(`sources.md`, `categories.md`, `BRIEFING.md`); state and archive live under `.claude-code-hermit/`.
...
- `--slot <name>`: custom slot label (e.g. `midday`). Use `<name>` verbatim wherever `<slot>` appears below. Orientation defaults to the morning (forward-looking) framing unless the operator's `BRIEFING.md` says otherwise.

Resolve `<slot>` from the flag before starting. All filenames below use that resolved value.

## Steps

1. Read `BRIEFING.md` — briefing philosophy, scoring rules, output format, and section guidance.
2. Read `sources.md` — full source list with types and URLs.
3. Read `categories.md` — operator's current focus areas and priority tiers (P1/P2).
4. Read `.claude-code-hermit/config.json` — the `briefing.enrichments.*` and `briefing.reaction_feedback` gates consulted in Phases 3 and 5b. A gate runs only when its value is `true`; treat an absent key as disabled.

### Phase 1 — Web/RSS sources

Dispatch the `@briefing-hermit:source-fetcher` subagent (model: haiku) to fetch all `web` and `rss`
sources from `sources.md`. Pass the full source list (URLs + names) and the resolved `<slot>`.

**Output-path contract:** the agent writes its extracted items to `tmp/briefing-source-items-<slot>.json`
...
- **`chrome` / `reddit-home` / `x`:** these require a running Chrome. Check whether the Chrome MCP tools are
  ...
  to launch a browser. If available, fetch per the operator's `BRIEFING.md` Chrome guidance (process
...
a. **Score and filter.** Apply the internal scoring in `BRIEFING.md`. Deduplicate across sources.
   Filter aggressively — penalize recycled discourse, reward concrete changes and primary sources.

b. **Story-arc cross-reference** — run only if `config.briefing.enrichments.story_arcs === true`.
...
c. **Follow-up CTA** — run only if `config.briefing.enrichments.follow_up_cta === true`.
   For every top-tier item (Read now or equivalent) that falls in one of the operator's **P1 categories**
   (from `categories.md`), append a CTA as a new line directly after the item's last content line,
...
Write the brief per `BRIEFING.md` philosophy, tone, and format. Apply the slot's orientation
...
brief_path: .claude-code-hermit/briefings/YYYY-MM-DD-<slot>.md
---
<full brief text as delivered>
```
Do not retry the send. Archive continues normally in Phase 6 regardless of delivery status.

**Phase 5b — Message ID capture** — run only if `config.briefing.reaction_feedback === true` AND delivery
...
Write the brief to `.claude-code-hermit/briefings/YYYY-MM-DD-<slot>.md` (create the directory if absent).
...
title: "News Brief — YYYY-MM-DD (<slot>)"
created: YYYY-MM-DDTHH:MM:SS+HH:MM     # ISO 8601 with timezone
tags: [briefing]
top_categories: [Cat A, Cat B, Cat C]  # top 5 by item count, verbatim from categories.md
...
- **`top_categories`:** use only exact category names from `categories.md` — no abbreviations or invented
...
Write `.claude-code-hermit/compiled/briefing-summary-last-brief-<YYYY-MM-DD>.md` for session-context
injection. Overwrite if a file for today already exists (e.g. an earlier slot ran); do not delete prior
days' files — the injection mechanism picks the newest `type: briefing-summary` automatically.
...
structured data (titles, URLs, dates, summaries). Only fetch domains present in `sources.md`; the
