...
description: >-
  ...
  recent briefings, does targeted web research on its primary source, and delivers a
  focused ~10-15 line analysis via the configured channel. General follow-up by default;
  a security variant when the item is security-tagged. Invoke with /briefing-hermit:deep-dive <slug>.
---
...
1. **Resolve the item.** Glob `.claude-code-hermit/briefings/` for files from the last 14 days (sort by
...
- Only fetch URLs matching domains already in `sources.md`, or primary sources (CVE databases, vendor
