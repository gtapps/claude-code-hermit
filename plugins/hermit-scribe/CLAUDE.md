...
Native approval is the execution checkpoint for publication. Preserve the preview and validation; do not add a second conversational yes/no step. The gate is a static `permissions.ask` rule in `settings.json`, not a hook: the publishing modes are named verbs, so a rule can say what a hook would otherwise have to parse. A denied request must not be retried through another tool or route.
