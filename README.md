<p align="center">
  ...
  <a href="plugins/claude-code-hermit/CHANGELOG.md"><img src="https://img.shields.io/badge/version-1.3.2-green.svg" alt="Version 1.3.2" /></a>
...
The dashboard can focus on research, training, home automation, or whatever the agent is responsible for. Its renderer rebuilds the page from saved state on refresh. See [Artifacts](plugins/claude-code-hermit/docs/artifacts.md) for publishing and customization.
...
- **[`/spawn-session`](plugins/claude-code-hermit/skills/spawn-session/SKILL.md)** starts a background helper in its own Git worktree, watches it, and relays its report. Add `--rc` to pick up the helper from the Claude app.
- **[`/rc-gate`](plugins/claude-code-hermit/skills/rc-gate/SKILL.md)** opens a gate for starting new sessions from the Claude app or web interface. Each gets its own Git worktree; cleanup removes worktrees left by archived sessions while preserving uncommitted work.
